var app = angular.module('posts', ['ngResource', 'ui.bootstrap', 'textAngular']);
// Create a controller with name postsListController to bind to the grid section.
app.controller('postsViewController', function ($scope, $rootScope, $sessionStorage, $anchorScroll, $location, postService, commentService) {
    ...
    $scope.$on('viewComments', function (event, postId) {
    	...
    	commentService.get(listCommentsArgs, function (data) {
            ...
            for (var i = 0; i < $scope.comments.list.length; i++) {
            	...
            	commentHTML = commentHTML + '<tr><td>' + $scope.comments.list[i].content + '</td></tr>';
            }
            commentHTML += "</table>";
